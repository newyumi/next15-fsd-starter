...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      'fixed inset-0 z-50 bg-[rgba(213,217,224,0.80)] backdrop-blur-sm',
      className,
    )}
    {...props}
    ref={ref}
...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-background px-5 py-6 shadow-lg transition ease-in-out',
  {
    variants: {
      side: {
  ...
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      className={cn(sheetVariants({ side }), className, {
        'rounded-tl-[32px] rounded-tr-0': side === 'bottom',
